...
### 3.2. El Bucle de Trabajo Obligatorio: `Leer -> Planificar -> Actuar -> Verificar -> Registrar`
Siga este bucle de 5 pasos para toda acción no trivial.
...
Usted **DEBE** mantener un registro forense de cada acción en `logs/bitacora_agente.md`. Use las plantillas completa (operaciones críticas) y resumida (micro-tareas) definidas en `PLAN_V6.md`.
...
*   **Credenciales:** El token `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` se proporciona para esta tarea.
...
Consulte **siempre** `base_de_conocimiento/metodologia/25_plan_de_trabajo_y_progreso.md` para las tareas pendientes y `.../20_protocolo_investigacion.md` para la metodología completa.
...
*   **`config.yaml`**: Fichero de configuración central para parámetros del proyecto (países, pesos de índices, filtros). Permite la modularidad y ajuste del análisis sin modificar el código.
*   **`dependencies.md`**: Documento crítico que detalla todas las dependencias técnicas del proyecto, incluyendo librerías de Python y herramientas del entorno.
*   **`ejemplo_informe_final.md`**: Una plantilla y ejemplo de cómo debe ser el informe final para una localidad, mostrando la estructura y el tipo de análisis cualitativo y cuantitativo esperado.
...
Contiene análisis temáticos más profundos.
...
*   **`README.md`**: El manual didáctico para la suite de herramientas.
